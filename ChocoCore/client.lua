...
RegisterCommand( "dv", function()
    TriggerEvent( "ChocoCore:deleteVehicle" )
end, false )
...
RegisterNetEvent( "ChocoCore:deleteVehicle" )
AddEventHandler( "ChocoCore:deleteVehicle", function()
...
RegisterNetEvent("ChocoCore:drag")
AddEventHandler("ChocoCore:drag", function(Dragger)
...
RegisterCommand("drag", function(source, args, fullCommand)
	...
	if Distance ~= -1 and Distance < Drag.Distance then
		TriggerServerEvent("ChocoCore:drag", GetPlayerServerId(Player))
	else
...
-- Crouch script
    ...
    --                         SetPedMovementClipset( ped, "move_ped_crouched", 0.85 )
    --                         crouched = true 
    --                     end 
    --                 end
    --             end 
...
RegisterNetEvent('ChocoCore:sendMessageShowID')
AddEventHandler('ChocoCore:sendMessageShowID', function(id, name1, name2)
  local myId = PlayerId()
		...
		if playerPed then
			...
			if currentPos == prevPos then
				if time > 0 then
					...
					TriggerServerEvent("ChocoCore:afkkick")
				end
			else
